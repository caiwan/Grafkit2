#include <grafkit/threadpool/AlignedMalloc.h>
...
void* AlignedMalloc(size_t size, size_t alignment)
{
	return _aligned_malloc_dbg(
		size, alignment, __FILE__, __LINE__); // This is reduced to a call to `_aligned_malloc` when _DEBUG is not defined
}
...
#if defined(__APPLE__) || defined(__linux__)
#include <stdlib.h>
void AlignedFree(void* pointer) { free(pointer); }
...
// https://sites.google.com/site/ruslancray/lab/bookshelf/interview/ci/low-level/write-an-aligned-malloc-free-function
#include <stdlib.h>
void aligned_free(void* pointer) { free(((void**)pointer)[-1]); }
