	...
	switch (uMsg)
	{
		case WM_INITDIALOG: {
			...
			while (1)
			{
				...
				/*** You can use this following line to select only certain aspect
				 * ratios ***/
				//    if ((d.dmPelsWidth * 3) / 4 != d.dmPelsHeight) continue;

				/*** You can use this following line to avoid "rotated" pictures on e.g.
				 * laptops ***/
				...
				if (!nRes || ress[nRes - 1].w != d.dmPelsWidth || ress[nRes - 1].h != d.dmPelsHeight)
				{
					...
					nRes++;
					_snprintf(st, 500, "%d * %d", d.dmPelsWidth, d.dmPelsHeight);
					SendDlgItemMessage(hWnd, IDC_RESOLUTION, CB_ADDSTRING, 0, (LPARAM)st);
				}
			}

			int s = nRes - 1;
			for (i = 0; i < nRes; i++)
				if (ress[i].w == setupcfg->scrWidth && ress[i].h == setupcfg->scrHeight) s = i;
			...
			if (setupcfg->nVsync) SendDlgItemMessage(hWnd, IDC_VSYNC, BM_SETCHECK, 1, 1);
			if (setupcfg->nMultisample) SendDlgItemMessage(hWnd, IDC_MULTI, BM_SETCHECK, 1, 1);
		}
		break;
		case WM_COMMAND:
